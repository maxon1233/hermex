...
final class SessionListMutationTests: XCTestCase {
    ...
    func testSessionsSurfaceExcludesCronExecutionsExceptExplicitProjectScope() async throws {
        let viewModel = try makeViewModel { request in
            ...
            {
              "sessions": [
                {"session_id":"ordinary-1","title":"Needle ordinary","project_id":"project-1","updated_at":5},
                {"session_id":"ordinary-2","title":"Other ordinary","project_id":"project-2","updated_at":4},
                {"session_id":"cron_1","title":"Needle scheduled 1","project_id":"project-1","updated_at":10},
                {"session_id":"cron_2","title":"Needle scheduled 2","project_id":"project-1","updated_at":20},
                {"session_id":"tagged-cron","title":"Needle tagged","source_tag":"cron","project_id":"project-1","updated_at":30}
              ]
            }
            """, for: request)
        }
        ...
        await viewModel.load()
        let visibility = AutomatedSessionVisibility.sessionsSurface(
            showsCli: true,
            showsClaudeCode: true,
            showsSubagents: false
        )
