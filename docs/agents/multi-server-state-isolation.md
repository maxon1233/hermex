...
| "Show CLI sessions" toggle | UserDefaults, per-server key (`SessionRowDisplaySettings.showCliSessionsKey(for:)` = `sessionRow.showCliSessions|<server absoluteString>`) | Per-server since #19: on Settings load a server `false` is adopted (hidden, as it is everywhere on desktop) while `true` only enables write-back — the server key means *availability* (desktop's separate CLI tab), so it never forces CLI rows into mobile's single list. User toggles write back via `POST /api/settings`; an adopted value on one server cannot leak to another. Reads fall back to the pre-#19 global key as a migration seed, then to hidden-by-default (the desktop main sidebar never mixes the CLI bucket in). Tested in `CliSessionsSyncModelTests`. |
